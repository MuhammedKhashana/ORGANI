...
import {Badge, Button, Card, Dialog, Flex, Select, Separator, Text, TextField} from "@radix-ui/themes";
import {Check2Square, CheckLg, ListCheck, PencilSquare, PlusLg, Trash3Fill} from "react-bootstrap-icons";
import Accordion from "react-bootstrap/Accordion";
import {addTask, deleteTaskX, editTask, isCompleted, tasks} from "../fnctios.js";
...
export default function MainTasks() {
	...
	function handleIsCompleted(data) {
		isCompleted(data);
		setTasks([...tasks]);
	}

	function deleteTask(idToDelete) {
		deleteTaskX(idToDelete);
		setTasks(tasks);
	}
		...
		<div>
			<Fade duration={"500"}>
				<div className='my-3 d-flex flex-row justify-content-between align-items-center'>
					<Text className='' size='3' weight='bold'>
						Your Tasks
					</Text>
					<Dialog.Root>
						<Dialog.Trigger className='d-flex justify-content-end'>
							<Button variant='surface' color='gray' className='mx-0 p-0'>
								<PlusLg size='20' className='mx-2' />
							</Button>
						</Dialog.Trigger>
						...
						<Dialog.Content maxWidth='450px'>
							...
							<Flex direction='column' gap='3'>
								...
								<div className='w-100 mt-2 d-flex flex-row justify-content-between align-items-center'>
									<label className='w-50 d-flex flex-row justify-content-between align-items-center'>
										<Text className='w-25' as='div' size='2' mb='1' weight='bold'>
											Date
										</Text>
										<TextField.Root
											className='w-75'
									...
									<Select.Root defaultValue='normal'>
										<Select.Trigger ref={priority} style={{width: "45%"}} />
										<Select.Content>
											<Select.Group>
												<Select.Label>Priority</Select.Label>
												<Select.Item value='weak'>weak</Select.Item>
										...
										onClick={() =>
											handleAddTask({
												...
												data: {
													...
													priority: priority.current.children[0].children[0].innerHTML,
			...
			<Zoom duration={"300"}>
				{taskS.length !== taskS.filter((task) => task.data.isCompleted == true).length ? (
					<Card>
						{taskS
							.filter((task) => task.data.isCompleted === false)
							.map((task) => (
								<Fade key={task.id} duration={"300"} cascade>
									<div className='mb-3'>
										<div className='d-flex flex-row justify-content-between align-items-center'>
											<div className='d-flex flex-column justify-content-between'>
												<Text size={"4"} weight={"meduim"}>
													{task.data.title}
												</Text>
												<div>
													<Badge mr={"3"} color={task.data.priority === "normal" ? "indigo" : task.data.priority === "weak" ? "green" : "tomato"}>
														<Text size={"1"} weight={"light"}>
															{task.data.priority}
														</Text>
													</Badge>
													<Text size={"2"} weight={"light"}>
														{ new Date(task.data.date).toDateString()}
													</Text>
												</div>
											</div>
											<div>
													...
													<Dialog.Content maxWidth='450px'>
														...
														<Flex direction='column' gap='3'>
															<label>
																...
																<TextField.Root ref={newTaskTitle} defaultValue={task.data.title} placeholder='Enter Your Task Title' />
															</label>
															<div className='w-100 mt-2 d-flex flex-row justify-content-between align-items-center'>
																<label className='w-50 d-flex flex-row justify-content-between align-items-center'>
																	<Text className='w-25' as='div' size='2' mb='1' weight='bold'>
																	...
																	<TextField.Root
																		...
																		ref={newDate}
																		defaultValue={
																			`${new Date(task.data.date).getFullYear()}` +
																			"-" +
																			`${String(new Date(task.data.date).getMonth() + 1).padStart(2, "0")}` +
																			"-" +
																			`${String(  new Date(task.data.date).getDate()).padStart(2, "0")}`
																		}
																		placeholder='Enter Your Task Description'
																	/>
																</label>
																<Select.Root defaultValue={task.data.priority}>
																	<Select.Trigger ref={newPriority} style={{width: "45%"}} />
																	<Select.Content>
																		<Select.Group>
																			<Select.Label>Priority</Select.Label>
																			<Select.Item value='weak'>weak</Select.Item>
															...
															<Dialog.Close>
																<Button
																	onClick={() =>
																		handleUpdateTask({
																			id: task.id,
																			newTitle: newTaskTitle.current.value,
																			newPriority: newPriority.current.children[0].children[0].innerHTML,
																			newDate: new Date(newDate.current.value),
																		})
																	}
																>
																	Save
												...
												<button
													...
													onClick={() =>
														handleIsCompleted({
															id: task.id,
															statu: !task.data.isCompleted, // Toggle the status
														})
													}
												>
						...
						<Accordion Accordion defaultActiveKey=''>
							<Accordion.Item eventKey='0' className=''>
								<Accordion.Header className='p-0 m-0'>
									...
									Completed Tasks {taskS.filter((task) => task.data.isCompleted === true).length}
								</Accordion.Header>
								<Accordion.Body className=''>
									<Fade duration={"300"} cascade>
										{taskS
											.filter((task) => task.data.isCompleted === true)
											.map((task) => (
												<div key={task.id} className='mb-3'>
													<div className='d-flex flex-row justify-content-between align-items-center'>
														<div className='d-flex flex-row justify-content-between align-items-center'>
															...
															<Text size={"4"} weight={"meduim"} className='opacity-50 text-decoration-line-through'>
																{task.data.title}
															</Text>
														</div>
														<div>
